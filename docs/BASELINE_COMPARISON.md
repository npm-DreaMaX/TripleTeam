...
新增的 `assurance` 也应在以上策略对照中保持相同。另做独立验证消融时，显式比较 `assurance.mode: "required"` 与 `"off"`，保留相同外部验收及共享总预算，计入规格设计、批判和复验用量。记录它额外拦截的错误候选、检查自身失效/不稳定、严格完成率和总成本；两次上下文调用本身不能当作质量提升的证据。
