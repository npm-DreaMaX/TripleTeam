...
import { writeZipArchive } from "../utils/zip.js";
import { completeSummarization, estimateTokens, getSummarizationFailure } from "./compaction/compaction.js";
...
export function redactJsonValue(value) {
    ...
    return JSON.parse(JSON.stringify(value, (key, child) => {
        if (child !== null && child !== undefined && isSensitiveKey(key))
            return REDACTED;
...
export function collectBugReportMetadata(options) {
...
export function collectBugReportDiagnostics(sessionManager, crashes = []) {
...
export function bugReportFiles(bundle) {
    const files = [
...
export function writeBugReportArchive(bundle, filePath) {
    return writeZipArchive(filePath, bugReportFiles(bundle));
}
export function bugReportArchiveFileName(id) {
...
export async function generateBugReportSummary(options) {
    ...
    const response = await completeSummarization(model, normalizeContext({
