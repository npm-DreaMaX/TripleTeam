...
export function bashTool(output = {}) {
    return {
        name: "bash",
        ...
        replay: "unsafe",
        output,
        async execute({ command, cwd }, api, ctx) {
            const started = Date.now();
            const child = spawn("bash", ["-c", command], { cwd, stdio: ["ignore", "pipe", "pipe"] });
            const onAbort = () => child.kill("SIGKILL");
            ctx.abortSignal?.addEventListener("abort", onAbort);
            child.stdout.on("data", (c) => api.stream(c));
            child.stderr.on("data", (c) => api.stream(c));
            const { code, signal } = await new Promise((r) => child.on("close", (code, signal) => r({ code, signal })));
