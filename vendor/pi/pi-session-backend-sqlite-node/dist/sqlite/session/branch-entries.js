...
import { decodeEntryRow } from "./entries.js";
function readBranchMembership(db, sessionId, entryId) {
    const row = sql `SELECT b.branch_id, b.entry_seq
		...
		LIMIT 1`.get(db);
    if (row === undefined)
        throw new Error(`Branch cache missing entry ${entryId}`);
    return row;
}
function readBranchMeta(db, sessionId, branchId) {
    const row = sql `SELECT branch_id, tip_entry_id, tip_seq, base_branch_id, base_seq
		FROM branch_meta
		WHERE session_id = ${sessionId} AND branch_id = ${branchId}`.get(db);
    if (row === undefined)
        throw new Error(`Branch metadata missing for branch ${branchId}`);
    return row;
}
function insertBranchEntry(db, sessionId, branchId, entry) {
...
function readBranchTipForParent(db, sessionId, parentId) {
    return sql `SELECT branch_id
		FROM branch_meta
		WHERE session_id = ${sessionId} AND tip_entry_id = ${parentId}`.get(db);
}
function createRootBranchForEntry(db, sessionId, entry) {
...
function appendEntryToExistingBranch(db, sessionId, branchId, entry) {
    ...
    if (result.changes !== 1)
        throw new Error(`Expected to update branch ${branchId}, updated ${result.changes}`);
}
function readBranchSegmentsNewestFirst(db, sessionId, start) {
    ...
    while (true) {
        ...
        if (meta.base_branch_id === null)
            break;
        if (meta.base_seq === null)
            throw new Error(`Branch ${branchId} has base branch without base_seq`);
        branchId = meta.base_branch_id;
        upperSeq = meta.base_seq;
    }
    return segments;
}
function readNewestCompactionBoundary(db, sessionId, segmentsNewestFirst) {
...
function createDivergentBranchForEntry(db, sessionId, entry) {
    if (entry.parentId === null)
        throw new Error("Root entries do not create divergent branches");
    ...
    sql `INSERT INTO branch_meta (session_id, branch_id, tip_entry_id, tip_seq, base_branch_id, base_seq)
		VALUES (${sessionId}, ${branchId}, ${entry.id}, ${entry.seq}, ${compaction?.branchId ?? null}, ${compaction?.seq ?? null})`.run(db);
    copyBranchEntriesAfterSeqThroughParent(db, sessionId, branchId, segmentsNewestFirst, compaction?.seq ?? 0);
    insertBranchEntry(db, sessionId, branchId, entry);
}
export function appendEntryToBranchIndex(db, sessionId, entry) {
    if (entry.parentId === null) {
...
function stopPredicates(query) {
    const predicates = [];
    if (query.stopAtType !== undefined)
        predicates.push(sql `b.entry_type = ${query.stopAtType}`);
    if (query.stopAtId !== undefined)
        predicates.push(sql `b.entry_id = ${query.stopAtId}`);
    return predicates;
}
function readStopSeq(db, sessionId, segment, query, oldestFirst) {
    ...
    const row = sql `SELECT ${aggregate} AS stop_seq
		...
		WHERE b.session_id = ${sessionId}
			...
			AND (${joinSqlFragments(stop, " OR ")})`.get(db);
    return row?.stop_seq ?? undefined;
}
function branchScanPredicates(sessionId, segment, query, oldestFirst, stopSeq) {
    const predicates = [
    ...
    if (query.customType !== undefined)
        predicates.push(sql `e.custom_type = ${query.customType}`);
    if (query.cursor !== undefined) {
        predicates.push(oldestFirst ? sql `b.entry_seq > ${query.cursor.seq}` : sql `b.entry_seq < ${query.cursor.seq}`);
    }
    return predicates;
}
function limitSql(limit) {
    return limit === undefined ? sql `` : sql `LIMIT ${Math.max(0, limit)}`;
}
function scanEntrySegmentRows(db, sessionId, segment, query, oldestFirst, stopSeq, limit) {
    ...
    return sql `SELECT e.id, e.parent_id, e.seq, e.type, e.custom_type, e.timestamp, e.payload
		...
		ORDER BY b.entry_seq ${order} ${limitSql(limit)}`.all(db);
}
function decodeEntryStructureRow(row) {
    return {
        id: row.id,
...
function scanStructureSegmentRows(db, sessionId, segment, query, oldestFirst, stopSeq, limit) {
    const predicates = branchScanPredicates(sessionId, segment, query, oldestFirst, stopSeq);
    const order = oldestFirst ? sql `ASC` : sql `DESC`;
...
function scanBranchSegments(db, sessionId, query, readSegment) {
...
export function scanBranchEntries(db, sessionId, query) {
    return scanBranchSegments(db, sessionId, query, scanEntrySegmentRows).map(decodeEntryRow);
}
export function scanBranchEntryStructures(db, sessionId, query) {
